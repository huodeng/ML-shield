...
class MemoryPoisonDataset(Dataset):
    def __init__(self, x_data, y_data, transform=None, 
                 trigger_path='packages/server/shield/BACKDOOR/triggers/trigger_white.png', 
                 trigger_size=5, trigger_label=0, poison_rate=0.1):
        self.images = x_data
...
def train_on_memory_data(
    ...
    # 保存模型
    os.makedirs("packages/server/shield/BACKDOOR/checkpointss", exist_ok=True)
    torch.save(model1.state_dict(), "packages/server/shield/BACKDOOR/checkpointss/backdoor_model.pth")
    print("模型已保存至 packages/server/shield/BACKDOOR/checkpointss/backdoor_model.pth")

def custombackdoor(net, dataset,imgsize):
    print("未加隐私保护")
